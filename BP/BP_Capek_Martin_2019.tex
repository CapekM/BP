...
\abstractEN{This bachelor thesis describes design and development of API in technology node.js.

First part is focused on analysis of functional and non-functional requirements of the Virtual historical guide editor, analysis and selection of API design tools.
In the next part, I will design the API using tools from the analysis and with regard to the current state of the database of the project Dowry Towns of the Queens of Bohemia.
Finally, I will describe the development of the API, which will result in a functional prototype that will be tested.
}
...
\chapter{Vývoj}
    ...
    \section{Použité technologie}
        ...
        \subsection{GitLab}
            todo
    \section{Vývojářské prostředí pro OS~Linux}  \label{vyvProstredi}
...
import { createBasicUsers } from "./controller/user";
    ...
    \section{Implementace} \label{impl}
        Zde popíši jak vypadá prototyp Rest API pro Editor virtuální reality. Naše implementace navazuje na předchozí sekci \hyperref[vyvProstredi]{Vývojářské prostředí pro OS Linux}.
        
        Jelikož se na databázi v projektu Věnná města českých královen ještě pracuje, bylo rozhodnuto, že v prototypu budou pouze entity User a Model.
        ...
        Hesla uživatelů se momentálně ukládají v plaintaxtu, což není ideální.  
\chapter{Testování}
    Pro ověření funkčnosti našeho prototypu provedeme jednotkové testování. Jednotka v našem případě znamená koncový bod. Pro testování našeho API použijeme framework Jest.

    \section{Jest}
        Jest je framework pro testování JavaScriptových apilkací. Hodí se pro testování Node.js a typescriptu.
        Pro náš prototyp jsem vytvořil dva konfigurační soubory pro testování, které se liší pouze tím, že v jednom se analyzuje pokrytí kódu testy. Testy bez pokrytí se pouští příkazem:
            \begin{minted}{bash}
npm test
            \end{minted}
        A testy s pokrytím:
...
\begin{conclusion}
    ...
    V návrhu se vyskytl problém, že současná databáze Věnných měst českých královen nepostačuje požadavkům editoru virtuální reality, byl proveden návrh bez ohledu na současný stav databáze avšak implementace prototypu pokryla jen část návrhu. Tento prototyp je připraven na další rozšíření.
